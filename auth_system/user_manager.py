...
class UserManager:
    ...
    def __init__(self, users_db_path: str = "auth_system/users.json",
        ...
        # Email configuration (prefer explicit `email_config`; otherwise read from environment)
        if email_config:
...
Cloud Storage System Team
    ...
    def authenticate(self, username: str, password: str, send_email: bool = False) -> Dict:
        ...
        # Print OTP prominently to console
        print(f"\n{'='*70}")
        print(f"🔑 OTP CODE FOR '{username.upper()}': {otp}")
        ...
        return {
            "status": "otp_required",
            "message": "Password correct. Check terminal for OTP." if not email_sent else "Password correct. OTP sent to email (also check terminal).",
            "otp": None,
            "email": user['email'],
    ...
    def complete_login(self, username: str, otp: str) -> Dict:
        ...
        """
        print(f"\n🔑 Attempting OTP verification for '{username}'...")
